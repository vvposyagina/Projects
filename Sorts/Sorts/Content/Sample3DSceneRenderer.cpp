...
// Инициализирует параметры вида при изменении размера окна.
...
// Вызывается по одному разу для каждого кадра, поворачивает куб и рассчитывает модель и матрицы видов.
...
// Поворот модели 3D-куба на заданные углы в радианах.
...
// При отслеживании 3D-куб можно вращать вокруг его оси Y, отслеживая положение указателя относительно ширины экрана вывода.
...
void Sample3DSceneRenderer::Render()
{
	...
	// Задание экрана в качестве целевого объекта прорисовки.
	ID3D11RenderTargetView *const targets[1] = { m_deviceResources->GetBackBufferRenderTargetView() };
	context->OMSetRenderTargets(1, targets, m_deviceResources->GetDepthStencilView());

	// Подготовка постоянного буфера для передачи графическому устройству.
	...
	context->IASetIndexBuffer(
		m_indexBuffer.Get(),
		DXGI_FORMAT_R16_UINT, // Каждый индекс представляет собой 16-битное беззнаковое целое число (short).
		0
		);
	...
	// Прикрепление нашего шейдера вершин.
	context->VSSetShader(
		m_vertexShader.Get(),
		nullptr,
		0
	...
	// Передача постоянного буфера графическому устройству.
	...
	// Прикрепление нашего шейдера пикселей.
	context->PSSetShader(
		m_pixelShader.Get(),
		nullptr,
		0
...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	// После загрузки файла шейдера пикселей создаются шейдер и постоянный буфер.
	...
	// После загрузки куба объект готов к прорисовке.
